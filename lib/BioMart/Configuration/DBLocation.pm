...
sub openConnection {
  ...
  eval {
        ...
        #             );
        # 
        #             my $sth_htgt = $dbh->prepare('alter session set NLS_SORT=BINARY_CI'); $sth_htgt->execute();
        #             my $sth_htgt2 = $dbh->prepare('alter session set NLS_COMP=LINGUISTIC'); $sth_htgt2->execute();
        # 
        # } else {
            $dbh = DBI->connect(
        			  $dsn,
